...
class VirtualOrganisation(_ControllerBase):
    ...
    _mutable_parameters = frozenset(['id', 'description', 'enabled', 'pin',
                                     'vo_is_domain', 'name', 'vo_role', 'group_id',
                                     'automatic_join'])
    ...
    def create_vo_role(self, context, vo_role):
        ...
        group = group_api.create_group(context, {"name":virtual_organisation_role["name"]})["group"]
    ...
    def update_vo_role(self, context, vo_role_id, vo_role):
        virtual_organisation_role = self._normalize_dict(virtual_organisation_role)
        IdentityProvider.check_immutable_params(virtual_organisation_role)
